...
3. **This skill runs in the current directory.** It modifies `src/`, `public/`, `docs/`, and `scripts/` in the directory where it's invoked. This is typically called from within a project directory (e.g., `projects/my-project/`), ensuring complete isolation between concurrent clones.
4. Verify the base project builds: `npm run build`. The Next.js + shadcn/ui + Tailwind v4 scaffold should already be in place in the current directory. If not, tell the user to set it up first.
